...
- WeÅfve also integrated a Script Editor complete with syntax highlighting of Python code that makes it really convenient to edit scripts within the ``canvas.py`` standalone.
- For KL programmers, weÅfve added code completion within Canvas. WeÅfve got lots of ideas on how to continue to improve this, but itÅfs pretty handy as-is so we thought weÅfd share it as soon as possible. 
- Starting Fabric Engine 2.2 is about twice as fast as before.
- Drag-and-drop port reordering. You asked for it. We added it. YouÅfll love it.
...
- **Limitation**: Saving Presets wonÅft include the attached comment (FE-4900)
...
- Support for FabricÅfs Inline Drawing requires changes to MODO. We are working with The Foundry to provide a way for Fabric to draw into the MODO viewport in future releases of MODO and Fabric for MODO.
...
[FE-6410] - Changing a port name after imploding a graph doesnÅft crash anymore
...
[FE-5835] - Disconnect node ports and connections easily with the shorcut ÅgDÅh
...
[FE-6369] - Exposing execute ports doesnÅft force an additional evaluation anymore
...
[FE-6564] - Alembic doesnÅft crash anymore when reading certain Houdini point clouds
